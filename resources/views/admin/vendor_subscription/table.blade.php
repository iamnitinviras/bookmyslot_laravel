                        ...
                        <tr>
                            ...
                            <td>
                                @if (auth()->user()->user_type == App\Models\User::USER_TYPE_ADMIN)
                                    <form action="{{ route('admin.plans.destroy', ['plan' => $plan->plan_id]) }}" method="POST"
                                        autocomplete="off" class="data-confirm"
                                        data-confirm-message="{{ __('system.users.are_you_sure', ['name' => $plan->name]) }}"
                                        data-confirm-title="{{ __('system.crud.delete') }}" id="delete-form_{{ $plan->id }}">
                                        @csrf
                                        @method('DELETE')
                                @endif
