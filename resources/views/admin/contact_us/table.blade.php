            ...
            <tbody>
                @forelse ($contactUsEmails as $contactUsEmail)
                    <tr>
                        ...
                        <td><a data-url="{{url('get-rightbar-content')}}" data-id="{{$contactUsEmail->id}}"
                                data-action="contact-us" onclick="show_rightbar_section(this)"
                                href="javascript:void(0)">{{ __('system.fields.view') }}</a></td>
                        <td>
                            <form
                                action="{{ route('admin.contact-request.destroy', ['contact_request' => $contactUsEmail->id]) }}"
                                method="POST" autocomplete="off" class="data-confirm"
                                data-confirm-message="{{ __('system.contact_us.are_you_sure') }}"
                                data-confirm-title="{{ __('system.crud.delete') }}"
                                id="delete-form_{{ $contactUsEmail->id }}">
                                @csrf
                                @method('DELETE')
