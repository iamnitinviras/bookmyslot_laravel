...
<div class="row">
    <div class="col-md-4">
        @php($lbl_price = __('system.fields.price'))
        <div class="mb-3 form-group">
            <label class="form-label" for="price">{{ $lbl_price }} <span class="text-danger">*</span></label>
            {!! html()->text('price', old('price'))
            ->class('form-control')
            ->id('price')
            ->required()
            ->attribute('placeholder', $lbl_price)!!}
        </div>
        @error('price')
        <div class="pristine-error text-help">{{ $message }}</div>
        @enderror
    ...
    <div class="col-md-4">
        @php($lbl_number_of_months = __('system.fields.number_of_months'))
        <div class="mb-3 form-group">
            <label class="form-label" for="number_of_months">{{ $lbl_number_of_months }}</label>
            {!! html()->select('number_of_months', getNumberOfMonths())
...
<div class="row">
    ...
    <div class="col-md-4">
        ...
        <div class="mb-3 form-group @error('status') has-danger @enderror  @error('status') has-danger @enderror">
            <label class="form-label w-100" for="name">{{trans('system.payment_setting.status')}}</label>
