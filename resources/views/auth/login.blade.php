...
@section('content')
    <div class="auth-content my-auto">
        ...
        <form autocomplete="off" class="mt-4 pt-2 pristine-valid" action="{{ url('/login') }}" method="post" id="pristine-valid" novalidate>
            @csrf
            ...
            <div class="mb-4 form-group">
                <div class="form-floating form-floating-custom auth-pass-inputgroup @error('password') has-danger @enderror">
                    ...
                    <input type="password" class="form-control pe-5 " name="password" placeholder="{{ $lbl_password }}" required  data-pristine-required-message="{{ __('validation.required', ['attribute' => strtolower($lbl_password)]) }}" data-pristine-password-message="{{ __('validation.password.invalid') }}" value="">
        ...
        <div class="mt-3 text-center">
           <p class="text-muted mb-0">{{ __('auth.login.no_account') }} <a href="{{ route('register') }}" class="text-primary fw-semibold">{{ __('auth.login.register_membership') }}</a> </p>
        </div>
    </div>
@endsection
